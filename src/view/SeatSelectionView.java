...
import model.Seat;
import model.User;
import model.StudyCafeRepository; // 조원 레포지토리 임포트
import javax.swing.*;
import java.awt.*;
...
public class SeatSelectionView extends JFrame {
    ...
    public SeatSelectionView(SeatController seatController, StudyCafeRepository repository, Collection<Seat> seats, SwingNavigator navigator) {
        ...
        setTitle("좌석 배치도");
        // 하단에 듬직한 메인 메뉴 버튼 공간이 추가되므로 세로 크기를 450에서 520으로 확장합니다!
        setSize(450, 520); 
        setLocationRelativeTo(null);
        
        // [레이아웃 튜닝] 전체 창을 BorderLayout으로 세팅!
        setLayout(new BorderLayout(10, 10)); 

        // 1. 🛠️ 좌석 16개를 묶어줄 바둑판 전용 주머니(Panel) 생성
        ...
        for (int i = 0; i < 16; i++) {
            ...
            seatPanel.add(seatButtons[i]); // 전체 창이 아니라 바둑판 주머니에 버튼을 넣습니다!
        }
        
        // 완성된 바둑판 주머니를 창의 '정중앙'에 챡 배치!
        ...
        mainBtn.setBackground(new Color(230, 242, 255)); // 다른 화면들과 통일감을 준 하늘색 톤!
        mainBtn.setPreferredSize(new Dimension(450, 50)); // 버튼 높이를 50으로 줘서 누르기 편하게 만듦!
        
        mainBtn.addActionListener(e -> {
            // 1. 현재 열려있는 좌석 화면을 보이지 않게 닫고 메모리 해제
            this.setVisible(false);
            this.dispose();
            
            // 2. ⚡ [먹통 방지 & 꺼짐 방지 완벽 해결]
            // 복제본 껍데기 화면을 새로 파는 게 아니라, 넘겨받은 원본 총사령관 리모컨을 작동시킵니다!
            if (navigator != null) {
        ...
        for (Seat seat : seats) {
            ...
            if (seat.isOccupied()) { 
                ...
                if (phone != null && !phone.isEmpty() && repository != null) {
                    ...
                    if (user != null) {
                        int hours = user.getRemainingHours();
                        ...
                        // 💡 혹시 모를 비회원이나 데이터 누락을 대비한 예외 안내 멘트 방어막!
                        timeText = "이용 중"; 
                    }
                } else {
                    timeText = "시간 정보 없음";
